...
OesClientApi::RptThreadMain(void *pParams) {
    ...
    volatile int32      *pRptChannelBrokenFlag = &pApi->rptChannelBrokenFlag;
...
CHECK_RPT_CHANNEL:
    /* 检查回报通道连接状态 */
    while (! *pTerminatedFlag) {        /* 检查客户端实例退出标志 */
        if (*pRptChannelBrokenFlag) {   /* 检查回报通道连接状态标志, 重连成功后会重置此标志 */
            SPK_SLEEP_MS(1);
        } else {
            break;
        }
            ...
            if (SPK_IS_NEG_ETIMEDOUT(ret)) {
                ...
                if (pRptChannel->heartBtInt > 0) {
                    ...
                    if (recvInterval > pRptChannel->heartBtInt * 2) {
                        ...
                        /* 会话超时, 断开回报通道并标记回报通道连接状态标志 */
                        OesApi_Destory(pRptChannel);
                        *pRptChannelBrokenFlag = 1;
                        pSpi->OnDisconnected(ret, OESAPI_CHANNEL_TYPE_REPORT);
                        goto CHECK_RPT_CHANNEL;
                    }
                }
 ...
 */
BOOL
OesClientApi::ReconnectOrdChannel(int32 *pLastClSeqNo) {
    return OesApi_InitOrdChannel2(&apiEnv.ordChannel, &apiCfg.ordChannelCfg,
            pLastClSeqNo);
}
 ...
 */
BOOL
OesClientApi::ReconnectRptChannel(int64 lastRptSeqNum) {
    if (likely(OesApi_InitRptChannel2(&apiEnv.rptChannel,
            &apiCfg.rptChannelCfg, &apiCfg.subscribeInfo, lastRptSeqNum))) {
        /* 重连成功, 重置回报通道状态标志 */
        rptChannelBrokenFlag = 0;
        return TRUE;
    }
 ...
 */
BOOL
OesClientApi::ReconnectQryChannel() {
    return OesApi_InitQryChannel2(&apiEnv.qryChannel, &apiCfg.qryChannelCfg);
}
...
OesClientApi::SendOrder(const OesOrdReqT *pOrderReq) {
    ...
    ret = OesApi_SendOrderReq(&apiEnv.ordChannel, pOrderReq);
    if (unlikely(ret < 0)) {
        /* 发送失败, 断开委托通道 */
        OesApi_Destory(&apiEnv.ordChannel);
        pSpi->OnDisconnected(ret, OESAPI_CHANNEL_TYPE_ORDER);
    }
...
OesClientApi::SendCancelOrder(const OesOrdCancelReqT *pCancelReq) {
    ...
    ret = OesApi_SendOrderCancelReq(&apiEnv.ordChannel, pCancelReq);
    if (unlikely(ret < 0)) {
        /* 发送失败, 断开委托通道 */
        OesApi_Destory(&apiEnv.ordChannel);
        pSpi->OnDisconnected(ret, OESAPI_CHANNEL_TYPE_ORDER);
...
OesClientApi::SendFundTrsf(const OesFundTrsfReqT *pFundTrsfReq) {
    ...
    ret = OesApi_SendFundTransferReq(&apiEnv.ordChannel, pFundTrsfReq);
    if (unlikely(ret < 0)) {
        /* 发送失败, 断开委托通道 */
        OesApi_Destory(&apiEnv.ordChannel);
        pSpi->OnDisconnected(ret, OESAPI_CHANNEL_TYPE_ORDER);
...
OesClientApi::QueryOrder(const OesQryOrdFilterT *pQryFilter, int32 requestId) {
    ...
    ret = OesApi_QueryOrder(&apiEnv.qryChannel, pQryFilter,
            _OesClientApi_QueryOrder, (void *) pSpi);
    if (unlikely(ret < 0)) {
        /* 查询收发失败, 断开查询通道 */
        OesApi_Destory(&apiEnv.qryChannel);
        pSpi->OnDisconnected(ret, OESAPI_CHANNEL_TYPE_QUERY);
    }
...
OesClientApi::QueryTrade(const OesQryTrdFilterT *pQryFilter, int32 requestId) {
    ...
    pSpi->currentRequestId = requestId;
    ret = OesApi_QueryTrade(&apiEnv.qryChannel, pQryFilter,
...
OesClientApi::QueryCashAsset(const OesQryCashAssetFilterT *pQryFilter, int32 requestId) {
    ...
    pSpi->currentRequestId = requestId;
    ret = OesApi_QueryCashAsset(&apiEnv.qryChannel, pQryFilter,
...
OesClientApi::QueryStkHolding(const OesQryStkHoldingFilterT *pQryFilter, int32 requestId) {
    ...
    pSpi->currentRequestId = requestId;
    ret = OesApi_QueryStkHolding(&apiEnv.qryChannel, pQryFilter,
...
OesClientApi::QueryLotWinning(const OesQryLotWinningFilterT *pQryFilter, int32 requestId) {
    ...
    pSpi->currentRequestId = requestId;
    ret = OesApi_QueryLotWinning(&apiEnv.qryChannel, pQryFilter,
...
OesClientApi::QueryCustInfo(const OesQryCustFilterT *pQryFilter, int32 requestId) {
    ...
    pSpi->currentRequestId = requestId;
    ret = OesApi_QueryCustInfo(&apiEnv.qryChannel, pQryFilter,
...
OesClientApi::QueryInvAcct(const OesQryInvAcctFilterT *pQryFilter, int32 requestId) {
    ...
    pSpi->currentRequestId = requestId;
    ret = OesApi_QueryInvAcct(&apiEnv.qryChannel, pQryFilter,
...
OesClientApi::QueryCommissionRate(const OesQryCommissionRateFilterT *pQryFilter, int32 requestId) {
    ...
    pSpi->currentRequestId = requestId;
    ret = OesApi_QueryCommissionRate(&apiEnv.qryChannel, pQryFilter,
...
OesClientApi::QueryFundTransferSerial(const OesQryFundTransferSerialFilterT *pQryFilter, int32 requestId) {
    ...
    pSpi->currentRequestId = requestId;
    ret = OesApi_QueryFundTransferSerial(&apiEnv.qryChannel, pQryFilter,
...
OesClientApi::QueryIssue(const OesQryIssueFilterT *pQryFilter, int32 requestId) {
    ...
    pSpi->currentRequestId = requestId;
    ret = OesApi_QueryIssue(&apiEnv.qryChannel, pQryFilter,
...
OesClientApi::QueryStock(const OesQryStockFilterT *pQryFilter, int32 requestId) {
    ...
    pSpi->currentRequestId = requestId;
    ret = OesApi_QueryStock(&apiEnv.qryChannel, pQryFilter,
...
OesClientApi::QueryEtf(const OesQryEtfFilterT *pQryFilter, int32 requestId) {
    ...
    pSpi->currentRequestId = requestId;
    ret = OesApi_QueryEtf(&apiEnv.qryChannel, pQryFilter,
...
OesClientApi::QueryEtfComponent(const OesQryEtfComponentFilterT *pQryFilter, int32 requestId) {
    ...
    pSpi->currentRequestId = requestId;
    ret = OesApi_QueryEtfComponent(&apiEnv.qryChannel, pQryFilter,
