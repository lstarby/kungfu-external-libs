...
public:
    /* 连接异常断开回调函数 */
    virtual void        OnDisconnected(int32 errorCode, eOesApiChannelTypeT channelType) = 0;
...
public:
    ...
    void                Stop(void);
    /* 重连委托通道 */
    BOOL                ReconnectOrdChannel(int32 *pLastClSeqNo = NULL);
    /* 重连回报通道 */
    BOOL                ReconnectRptChannel(int64 lastRptSeqNum = -1);
    /* 重连查询通道 */
    BOOL                ReconnectQryChannel();
...
private:
    ...
    volatile int32      rptChannelBrokenFlag;
